...
out_mu = np.average(out_avg_mu)
out_sigma = np.average(np.sqrt(out_avg_var_mu))

cat_mu = np.average(cat_avg_mu)
cat_sigma = np.average(np.sqrt(cat_avg_var_mu))

hand_mu = np.average(hand_avg_mu)
hand_sigma = np.average(np.sqrt(hand_avg_var_mu))
...
c_patch = mpatches.Patch(color='cyan', label='"Cat"')
r_patch = mpatches.Patch(color='red', label='"Flipped"')
...
plt.plot(x, stats.norm.pdf(x, hand_mu, hand_sigma), 'm')
plt.show()
plt.savefig("in_pdf2.png", bbox_inches='tight')

x = np.linspace(in_mu - 10*np.sqrt(in_sigma), in_mu + 10*np.sqrt(in_sigma), 1000)
plt.plot(x, stats.norm.pdf(x, in_mu, in_sigma), 'g')
plt.plot(x, stats.norm.pdf(x, out_mu, out_sigma), 'b')
plt.plot(x, stats.norm.pdf(x, flipped_mu, flipped_sigma), 'c')
plt.plot(x, stats.norm.pdf(x, cat_mu, cat_sigma), 'r')
